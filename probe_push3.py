"""Disambiguate 'batch reuse' from 'more Adam steps' in BP.

BP K=64 reached 0.031 (vs LocoProp n=256 at 0.019). Two controls:
- BP K=256 (more reuse — does it keep helping or saturate?)
- BP K=1 at 4x/16x outer steps (fresh data, same total Adam step count as K=4/16)
"""
import time
import numpy as np
import torch
...
configs = [
    # More batch reuse
    dict(label='BP K=256 lr=1e-3 ep=2048',  k=256, lr=1e-3, epochs=2048),
    dict(label='BP K=256 lr=3e-4 ep=2048',  k=256, lr=3e-4, epochs=2048),
    # Fresh-data matched total Adam steps: K=1 at 16x, 64x horizon
