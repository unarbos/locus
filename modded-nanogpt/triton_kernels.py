...
# nvrtc accepts the major SM family; Blackwell's sub-versions (e.g. sm_103) fall back to sm_100.
_cap_str = f"{_cap[0]}0" if _cap[0] >= 10 else f"{_cap[0]}{_cap[1]}"
ce_fwd_bwd_kernel = torch.cuda._compile_kernel(
    CE_KERNEL_DECLS + CE_KERNEL_SOURCE,
    "ce_fwd_bwd_kernel",
    compute_capability=_cap_str,
